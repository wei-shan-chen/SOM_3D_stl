...
bool STLmodel::LoadSTLfile(const char *stlFileName)
{
    ...
    while (1)
    {
        ...
        // record max min that can easy to scale、translate、rotate
        {
            if (triangleNum == 0)
            {
                m_MaxPos.x = m_MinPos.x = tri1.position.x;
                m_MaxPos.y = m_MinPos.y = tri1.position.y;
                m_MaxPos.z = m_MinPos.z = tri1.position.z;
            }
            triangleNum += 3;

            // maximum
            if (m_MaxPos.x < tri1.position.x)
                m_MaxPos.x = tri1.position.x;
            if (m_MaxPos.x < tri2.position.x)
                m_MaxPos.x = tri2.position.x;
            if (m_MaxPos.x < tri3.position.x)
                m_MaxPos.x = tri3.position.x;
            if (m_MaxPos.y < tri1.position.y)
                m_MaxPos.y = tri1.position.y;
            if (m_MaxPos.y < tri2.position.y)
                m_MaxPos.y = tri2.position.y;
            if (m_MaxPos.y < tri3.position.y)
                m_MaxPos.y = tri3.position.y;
            if (m_MaxPos.z < tri1.position.z)
                m_MaxPos.z = tri1.position.z;
            if (m_MaxPos.z < tri2.position.z)
                m_MaxPos.z = tri2.position.z;
            if (m_MaxPos.z < tri3.position.z)
                m_MaxPos.z = tri3.position.z;
            // minimum
            if (m_MinPos.x > tri1.position.x)
                m_MinPos.x = tri1.position.x;
            if (m_MinPos.x > tri2.position.x)
                m_MinPos.x = tri2.position.x;
            if (m_MinPos.x > tri3.position.x)
                m_MinPos.x = tri3.position.x;
            if (m_MinPos.y > tri1.position.y)
                m_MinPos.y = tri1.position.y;
            if (m_MinPos.y > tri2.position.y)
                m_MinPos.y = tri2.position.y;
            if (m_MinPos.y > tri3.position.y)
                m_MinPos.y = tri3.position.y;
            if (m_MinPos.z > tri1.position.z)
                m_MinPos.z = tri1.position.z;
            if (m_MinPos.z > tri2.position.z)
                m_MinPos.z = tri2.position.z;
            if (m_MinPos.z > tri3.position.z)
                m_MinPos.z = tri3.position.z;
        }
    ...
    m_scale = m_scale < (1.0 / (m_MaxPos.x - m_MinPos.x)) ? m_scale : (1.0 / (m_MaxPos.x - m_MinPos.x));
    m_scale = m_scale < (1.0 / (m_MaxPos.y - m_MinPos.y)) ? m_scale : (1.0 / (m_MaxPos.y - m_MinPos.y));
    m_scale = m_scale < (1.0 / (m_MaxPos.z - m_MinPos.z)) ? m_scale : (1.0 / (m_MaxPos.z - m_MinPos.z));
